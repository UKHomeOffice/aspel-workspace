...
const closedStatuses = ['resolved', 'rejected', 'refused', 'discarded-by-applicant', 'withdrawn-by-applicant'];
...
const isSuspension = task => {
  return get(task, 'data.action') === 'suspend';
};

const isReinstatement = task => {
  return get(task, 'data.action') === 'reinstate';
};
...
const taskHelper = {
  ...
  isIntentionToRefuseNotice: task => {
    ...
    return model === 'project' && action === 'grant' && event === 'status:with-inspectorate:intention-to-refuse';
  },
