...
export default function useTaskStatic(task = {}) {
  ...
  const {
    ...
    establishment: staticEstablishment,
  ...
  const taskEstablishment = task?.data?.establishment;
  ...
  const profileFromTask = staticProfile || task?.data?.modelData?.profile || staticValues;
  const modelData = task?.data?.modelData;
  const trainingTask = task?.data?.data;
  const trainingCourse = task?.data?.modelData?.trainingCourse;
